...
| `file` | string | No* | Base64-encoded file content (CSV, XLSX) |
| `data` | array | No* | JSON array of contact objects |
| `company_id` | string | No | Target company ID (defaults to authenticated user's company) |

`* Either file or data is required.`

::note
**Automatic company assignment.** The `company_id` is automatically set from your authentication token. You only need to provide it if you have permissions to import for another company.
::
