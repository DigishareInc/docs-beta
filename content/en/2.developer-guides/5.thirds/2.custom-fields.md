...
::card-group
::card{title="Quick Fields (Schema-less)" icon="i-mdi-flash"}
...
::card{title="Advanced API (Strict Schema)" icon="i-mdi-database"}
...
When importing bulk data via **Spreadsheets (CSV/Excel)**, the **Standard Bulk API**, or the **Ingestion API (Egressor)**, Digishare uses a flat structure with a prefix protocol to distinguish standard fields from custom metadata.

### The Prefix Protocol

Unlike the standard single-entity API where you nest fields under `data.custom_fields`, bulk operations require you to append the `cf_` or `custom_` prefix to the keys/columns. The engine will strip the prefix and bind the values as custom fields.

**Supported Bulk Methods:**
1. **Spreadsheets**: Column headers (e.g., `cf_balance`).
2. **JSON Bulk API**: Object keys in the `data` array (e.g., `"cf_balance": 150.75`).
3. **Ingestion API**: Object keys in the event payload (e.g., `"cf_balance": 150.75`).

| Key / Column Name | Final Slug | Logic |
| ----------------- | ---------- | ----- |
| `cf_balance`      | `balance`  | Prefixed (Auto-index) |
| `custom_vip`      | `vip`      | Prefixed (Auto-index) |
...
::card-group
::card{title="Bulk Import API" icon="i-mdi-upload" to="/developer-guides/thirds/import"}
Learn how to use the standard and ingestion bulk endpoints.
::
::card{title="CRUD Operations" icon="i-mdi-database" to="/developer-guides/thirds/crud"}
Reference for standard entity creation and updating.
::
::
